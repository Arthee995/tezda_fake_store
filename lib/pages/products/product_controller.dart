...
class ProductController extends GetConnect {
  ...
  Future<List<ProductModel>> fetchProductAPI() async {
    final response = await http.get(
      Uri.parse(AppConstants.BASE_URL + AppConstants.PRODUCTS_URL),
  ...
  Future<ProductModel> fetchProductDetailAPI({int? id}) async {
    final response = await http.get(
      Uri.parse(AppConstants.BASE_URL + AppConstants.PRODUCTS_URL + "/${id}"),
      headers: {"Content-Type": "application/json"},
    );
